...
try:
    ...
    raise ImportError('This script requires scipy. Please install it with `pip install scipy`.') from exc
EPS = 1e-12
P = 2
Q_LIST = [1, 3, 5]
ENERGY_RULES: List[Tuple[str, Optional[int]]] = [('C', None), ('e', None), ('r', 1), ('r', 2), ('r', 3), ('r', 4)]
ENERGY_ORDER = ['C', 'e', 'r=1', 'r=2', 'r=3', 'r=4']
SCENARIO_SHORT_LABELS = {'low_dim_linear': '2D affine line', 'low_dim_nonlinear': 'Circle vs flower', 'high_dim_linear': 'High-D linear', 'high_dim_nonlinear': 'High-D nonlinear'}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='GEBSW Wasserstein-fidelity heatmaps on manifold distributions.')
    ...
    parser.add_argument('--temperature_scale', type=float, default=1.0, help='Softmax temperature scale for e-rule: T = scale * std(costs).')
    parser.add_argument('--out_dir', type=str, default='gebsw_w2_fidelity_outputs', help='Output directory.')
    parser.add_argument('--plot_n', type=int, default=384, help='Number of points used only for distribution visualization panels.')
    parser.add_argument('--scenarios', nargs='+', default=['low_dim_linear', 'low_dim_nonlinear', 'high_dim_linear', 'high_dim_nonlinear'], choices=['low_dim_linear', 'low_dim_nonlinear', 'high_dim_linear', 'high_dim_nonlinear'], help='Distribution scenarios to run.')
...
def normalize_pair(X: np.ndarray, Y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    std = Z.std(axis=0, keepdims=True) + 1e-08
...
def make_low_dim_linear(n: int, seed: int, noise: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    X = X + noise * (0.35 * rng.normal(size=(n, 1)) * direction[None, :] + 0.15 * rng.normal(size=(n, 1)) * normal[None, :])
    Y = Y + noise * (0.35 * rng.normal(size=(n, 1)) * direction[None, :] + 0.15 * rng.normal(size=(n, 1)) * normal[None, :])
...
def make_swiss_roll(n: int, seed: int, noise: float, twist: bool) -> np.ndarray:
    ...
    X = X / (np.std(X, axis=0, keepdims=True) + 1e-08)
    if twist:
...
def make_high_dim_nonlinear(n: int, seed: int, noise: float, dim_high: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    A = A / (np.linalg.norm(A, axis=0, keepdims=True) + 1e-08)
    X = X3 @ A
    Y = Y3 @ A
    X = X + noise * rng.normal(size=X.shape)
    Y = Y + noise * rng.normal(size=Y.shape)
...
def energy_weights(costs: np.ndarray, rule: str, r: Optional[int], temperature_scale: float) -> np.ndarray:
    ...
    if rule == 'e':
        temp = temperature_scale * np.std(costs) + 1e-08
...
def gebsw_distance(X: np.ndarray, Y: np.ndarray, cfg: Config, L: int, seed: int, temperature_scale: float) -> Tuple[float, float, float]:
...
def plot_heatmap(pivot_values: pd.DataFrame, pivot_labels: Optional[pd.DataFrame], title: str, cbar_label: str, out_path: str, fmt: str='.3f') -> None:
    ...
    for i in range(pivot_values.shape[0]):
        for j in range(pivot_values.shape[1]):
            ...
            if is_best:
                ...
                ax.text(j, i, label_text, ha='center', va='center', fontsize=9, fontweight='bold', color=best_text_color, zorder=4)
                rect = plt.Rectangle((j - 0.5, i - 0.5), 1, 1, fill=False, edgecolor='black', linewidth=3.0, zorder=6)
                ax.add_patch(rect)
            else:
                ax.text(j, i, label_text, ha='center', va='center', fontsize=8.5, fontweight='normal', color=plain_text_color)
...
def representative_pair_for_plot(args: argparse.Namespace, scenario: str, scenario_idx: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    X, Y = make_pair(scenario=scenario, n=visual_n, seed=seed_pair, noise=args.noise, dim_high=args.dim_high)
...
def plot_distribution_compact(ax: plt.Axes, X: np.ndarray, Y: np.ndarray, scenario: str, show_legend: bool=False, w2_ref: Optional[float]=None) -> None:
    ...
    if w2_ref is not None:
        ax.text(0.5, -0.085, f'$W_2^{{\\mathrm{{ref}}}}={w2_ref:.4f}$', transform=ax.transAxes, ha='center', va='top', fontsize=7.2, fontweight='normal', clip_on=False)
    if show_legend:
        ax.legend(loc='best', fontsize=5.8, frameon=False, borderpad=0.15, handletextpad=0.25, markerscale=1.2)

def draw_compact_heatmap_on_ax(ax: plt.Axes, err_pivot: pd.DataFrame, labels: pd.DataFrame, w2_ref: float, vmin: float, vmax: float) -> matplotlib.image.AxesImage:
    ...
    for i in range(err_pivot.shape[0]):
        for j in range(err_pivot.shape[1]):
            ...
            ax.text(j, i, str(labels.values[i, j]), ha='center', va='center', fontsize=6.0 if not is_best else 6.5, fontweight='bold' if is_best else 'normal', color=text_color, linespacing=1.0, zorder=7 if is_best else 3)
            if is_best:
                rect = plt.Rectangle((j - 0.5, i - 0.5), 1, 1, fill=False, edgecolor='black', linewidth=3.0, zorder=6)
                ax.add_patch(rect)
    return im

def plot_sr_compact_2col4row(summary: pd.DataFrame, scenarios: Iterable[str], args: argparse.Namespace, out_path: str) -> None:
    valid_scenarios = [s for s in scenarios if not summary[summary['scenario'] == s].empty]
    if len(valid_scenarios) != 4:
        raise ValueError(f'The compact paper figure is designed for exactly 4 scenarios. Got {len(valid_scenarios)} valid scenarios: {valid_scenarios}')
    all_err_values = []
    ...
    gs = fig.add_gridspec(nrows=4, ncols=3, width_ratios=[0.68, 2.22, 0.05], height_ratios=[1, 1, 1, 1], hspace=0.4, wspace=0.16)
    last_im = None
    ...
    fig.subplots_adjust(left=0.055, right=0.965, top=0.985, bottom=0.06, hspace=0.4, wspace=0.16)
...
def make_plots(summary: pd.DataFrame, scenarios: Iterable[str], out_dir: str, args: argparse.Namespace) -> None:
    ...
    for scenario in scenarios:
        ...
        fidelity_title = f'GEBSW Wasserstein-fidelity heatmap: {scenario}\nExact empirical W2 reference = {w2:.4f}; color encodes relative error (lower is better)'
        fidelity_cbar = 'Relative error |D_GEBSW - W2| / W2'
        plot_heatmap(err_pivot, fidelity_label, title=fidelity_title, cbar_label=fidelity_cbar, out_path=os.path.join(plot_dir, f'fidelity_heatmap_{scenario}.png'))
        plot_distance_bar_with_reference(summary, scenario, out_path=os.path.join(plot_dir, f'distance_with_w2_reference_{scenario}.png'))
    plot_sr_compact_2col4row(summary, scenarios, args, out_path=os.path.join(plot_dir, 'sr_compact_distribution_heatmap_2col4row.png'))
...
def run(args: argparse.Namespace) -> None:
    ...
    for scenario in args.scenarios:
        ...
        for rep in range(args.repeats):
            ...
            X, Y = make_pair(scenario=scenario, n=args.n, seed=seed_pair, noise=args.noise, dim_high=args.dim_high)
            w2_ref = exact_empirical_w2(X, Y)
            theta_seed_by_q = {q: args.seed + 50000 * rep + 1009 * q + 17 * (args.scenarios.index(scenario) + 1) for q in Q_LIST}
            for cfg in configs:
                dist, entropy, top5 = gebsw_distance(X, Y, cfg, L=args.L, seed=theta_seed_by_q[cfg.q], temperature_scale=args.temperature_scale)
                abs_err = abs(dist - w2_ref)
                rel_err = abs_err / (abs(w2_ref) + EPS)
                raw_rows.append({'scenario': scenario, 'repeat': rep, 'q': cfg.q, 'energy_rule': cfg.energy_label, 'method': cfg.method_label, 'family': cfg.family, 'distance': dist, 'w2_reference': w2_ref, 'absolute_error': abs_err, 'relative_error': rel_err, 'weight_entropy_norm': entropy, 'top5_weight_ratio': top5, 'n': args.n, 'L': args.L, 'dim': X.shape[1]})
    raw = pd.DataFrame(raw_rows)
    raw_path = os.path.join(args.out_dir, 'raw_results.csv')
    raw.to_csv(raw_path, index=False)
    summary = raw.groupby(['scenario', 'q', 'energy_rule', 'method', 'family'], as_index=False).agg(distance_mean=('distance', 'mean'), distance_std=('distance', 'std'), w2_reference_mean=('w2_reference', 'mean'), w2_reference_std=('w2_reference', 'std'), absolute_error_mean=('absolute_error', 'mean'), absolute_error_std=('absolute_error', 'std'), relative_error_mean=('relative_error', 'mean'), relative_error_std=('relative_error', 'std'), entropy_mean=('weight_entropy_norm', 'mean'), top5_mean=('top5_weight_ratio', 'mean'))
    ...
    family_summary = raw.groupby(['scenario', 'family'], as_index=False).agg(distance_mean=('distance', 'mean'), relative_error_mean=('relative_error', 'mean'), relative_error_std=('relative_error', 'std'), entropy_mean=('weight_entropy_norm', 'mean'), top5_mean=('top5_weight_ratio', 'mean'))
