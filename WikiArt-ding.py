...
DPI = 600
BASE_DIR = "/home/zhouyan/EBGSW/Paired-WikiArt"
OUTPUT_BASE_DIR = "WikiArt-gradient-50L-10rep-350stepOpt-full"
N_PROJ = 50
N_REPEATS = 10
...
# ---------------------- 数据集配对配置 ----------------------
WIKIART_PAIRS = [
    ("1-esaias-van-de-velde_landscape-with-ruins.jpg",
     "11-abraham-manievich_flowers-on-a-yellow-table.jpg",
     "T1_pair_1_to_11"),
    ("2-canaletto_campo-sant-angelo.jpg",
     "22-alfred-sisley_bridge-at-villeneuve-la-garenne-1872.jpg",
     "T1_pair_2_to_22"),
    ("3-aleksey-savrasov_the-village-in-winter-1890.jpg",
     "33-a.y.-jackson_nellie-lake-1933.jpg",
     "T1_pair_3_to_33"),
    ("4-valentin-serov_portrait-of-anton-chekhov-1903.jpg",
     "44-allan-ramsay_general-sir-james-abercromby-also-spelled-abercrombie.jpg",
     "T2_pair_4_to_44"),
    ("5-alexey-venetsianov_reapers.jpg",
     "55-albert-bloch_portrait-of-a-boy.jpg",
     "T2_pair_5_to_55"),
    ("6-frans-hals_the-laughing-child.jpg",
     "66-carl-ludwig-johann-christineck_natalia-ozerova.jpg",
     "T2_pair_6_to_66"),
    ("7-philip-guston_celebration.jpg",
     "77-charles-lapicque_espagne.jpg",
     "T3_pair_7_to_77"),
    ("8-andre-pierre-arnal_pliage-folded-painting-1972.jpg",
     "88-arthur-dove_sunrise-1924.jpg",
     "T3_pair_8_to_88"),
    ("9-raoul-ubac_composition.jpg",
     "99-allan-d'arcangelo_landscape-bb-1974.jpg",
     "T3_pair_9_to_99"),
    ("111-denise-green_blu-lascaux-1999.jpg",
     "1111-albert-bloch_still-life-iii.jpg",
     "T4_pair_111_to_1111"),
    ("222-constantine-maleas_landscape.jpg",
     "2222-charles-reiffel_summer-design-1917.jpg",
     "T4_pair_222_to_2222"),
    ("333-gustave-loiseau_etretat-1902.jpg",
     "3333-ion-pacea_marina-4.jpg",
     "T4_pair_333_to_3333"),
]
...
def create_distance_functions(base_seed=2024, device=None):
    ...
    for p in [1, 2, 3, 4]:
        ...
        distances[key_name] = DistanceFunctionGPU(
            key_name,
            degree=5, energy_type='poly', energy_q=p, n_projections=N_PROJ // 2, proj_seed=base_seed + 400 + p, device=device
        )
    return distances
...
# 【修正】仅记录 step 与 loss，删除 grad_norm_per_pixel 及 RGB 梯度字段
def color_transfer_optimization(source_img, target_img, distance_func,
                                n_steps, lr, device, record_gradient_flow=False):
    ...
    gradient_flow = [] if record_gradient_flow else None
    ...
    for step in range(n_steps):
        ...
        if step % 50 == 0 and current_rgb.grad is not None:
            ...
            if torch.isnan(current_rgb.grad).any():
                print("  [警告] 检测到 nan 梯度，请检查 1D Wasserstein 输入！")
...
def run_experiment_for_pair(source_name, target_name, pair_id, repeat_idx, device, lpips_model):
    source_path = os.path.join(BASE_DIR, source_name)
    target_path = os.path.join(BASE_DIR, target_name)
    print(f"\n{'=' * 60}")
    print(f"[{device}] 处理配对: {pair_id} | 第 {repeat_idx + 1} 轮")
    ...
    for name, dist_func in tqdm(distance_functions.items(), desc=f"Processing {pair_id}"):
        try:
            ...
            results[name] = {
                ...
                'GradientFlow': grad_flow
            }
            print(f"{name:50s} | W2: {final_w2:.4f} | PSNR: {psnr_val:.2f} | "
...
def gpu_worker(gpu_id, pairs, shared_results):
    torch.cuda.set_device(gpu_id)
    device = torch.device(f"cuda:{gpu_id}")
    local_lpips = lpips.LPIPS(net='alex').to(device)
    local_lpips.eval()

    print(f"[GPU {gpu_id}] 工作进程启动，负责 {len(pairs)} 对图像")

    for source_name, target_name, pair_id in pairs:
        pair_results = {}
        for i in range(N_REPEATS):
            results, src_img, tgt_img = run_experiment_for_pair(
                source_name, target_name, pair_id, i, device, local_lpips
            )
            pair_results[f'Round_{i + 1}'] = results
        shared_results[pair_id] = pair_results
        print(f"[GPU {gpu_id}] 完成配对: {pair_id}")

    print(f"[GPU {gpu_id}] 全部任务完成")
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        print(f"检测到 {n_gpus} 块GPU:")
        for i in range(n_gpus):
            print(f"  GPU {i}: {torch.cuda.get_device_name(i)}")
    else:
        print("警告：未检测到GPU，将使用CPU运行（双GPU逻辑退化为单进程）")
        n_gpus = 1
    ...
    if USE_CUSTOM_PAIRS:
        ...
        if TYPE_INDEX == -1:
            ...
            selected_pairs = WIKIART_PAIRS[start_idx:end_idx]
            type_names = ["T1_pairs_1-3", "T2_pairs_4-6", "T3_pairs_7-9", "T4_pairs_111-333"]
            type_name = type_names[TYPE_INDEX]

    print("=" * 60)
    print("开始WikiArt数据集颜色迁移实验【双GPU并行版】")
    print(f"当前运行类型: {type_name} (索引: {TYPE_INDEX})")
    print(f"总配对数: {len(selected_pairs)}，每对重复 {N_REPEATS} 次")
    print("=" * 60)

    if n_gpus >= 2:
        mid = len(selected_pairs) // 2
        pairs_gpu0 = selected_pairs[:mid]
        pairs_gpu1 = selected_pairs[mid:]
    else:
        pairs_gpu0 = selected_pairs
        pairs_gpu1 = []
    ...
    processes = []
    if len(pairs_gpu0) > 0:
        p0 = mp.Process(target=gpu_worker, args=(0, pairs_gpu0, shared_results))
        p0.start()
        processes.append(p0)
    if len(pairs_gpu1) > 0:
        p1 = mp.Process(target=gpu_worker, args=(1, pairs_gpu1, shared_results))
        p1.start()
        processes.append(p1)
    ...
    all_results = dict(shared_results)

    print("\n" + "=" * 60)
    print("所有GPU实验完成，开始汇总与导出...")
    print("=" * 60)

    save_all_results(all_results)

    # ---------------------- 【修正】梯度流导出：宽表格式，单Sheet ----------------------
    print("\n" + "=" * 60)
    print("正在导出梯度流数据...")
    print("=" * 60)
    ...
    for pair_id, pair_data in all_results.items():
        for round_idx in range(N_REPEATS):
            round_key = f'Round_{round_idx + 1}'
            if round_key not in pair_data:
                continue

            # 按 Step 聚合所有变体的 loss
            step_dict = {}
            for dist_name in pair_data[round_key].keys():
                gf_list = pair_data[round_key][dist_name].get('GradientFlow')
                if not gf_list:
                    continue
                for entry in gf_list:
                    step = entry['step']
                    if step not in step_dict:
                        step_dict[step] = {'Step': step}
                    step_dict[step][dist_name] = entry['loss']
            ...
            # 确保 Step 为第一列，其余按变体原始名称排列
            cols = ['Step'] + [c for c in df.columns if c != 'Step']
            df = df[cols]

            # 文件名：若仅1轮则省略 Round 后缀，与历史逻辑兼容
            if N_REPEATS == 1:
                gf_path = os.path.join(gf_dir, f"{pair_id}_gradient_flow.xlsx")
            else:
                gf_path = os.path.join(gf_dir, f"{pair_id}_gradient_flow_Round{round_idx + 1}.xlsx")

            with pd.ExcelWriter(gf_path, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='Gradient_Flow', index=False)
            print(f"✓ 梯度流已保存: {gf_path}")

    print(f"\n梯度流导出完成！保存在: {gf_dir}")
    print("=" * 60)

    print("\n" + "=" * 60)
    print("实验全部完成！结果已保存到:", OUTPUT_BASE_DIR)
    ...
    print(f"  - {OUTPUT_BASE_DIR}/gradient_flow/   : 各配对的梯度流宽表（单Sheet，Step×变体loss）")
    print("=" * 60)