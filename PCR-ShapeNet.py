...
import h5py  # ShapeNet HDF5 support
...
# ======================== ShapeNet 数据集配置 ========================
# 保留 ModelNet40 framework-with-external-baselines 的实验设计：
# 18 个 GEBSW-family 配置 + Sampled-MaxSW / DSW 外部 baseline + 统一框架分析。
# 这里只将数据源替换为 shapenet-cgpt.py 使用的 ShapeNet Part HDF5 数据。
SHAPENET_HDF5_ROOT = "/home/zhouyan/EBGSW/shapenet_data/shapenet_part_seg_hdf5_data/hdf5_data"
NUM_PAIRS = 15
PAIR_SEED = 42
...
def load_shapenet_hdf5_all(h5_root):
    """读取所有 ShapeNet Part HDF5 文件，返回点云数组 (N, P, 3) 和类别标签 (N,)。"""
...
def generate_shapenet_pairs(all_pcs, all_labels, num_pairs=NUM_PAIRS, seed=PAIR_SEED,
                            cross_category=PAIR_CROSS_CATEGORY,
                            balanced_by_category=PAIR_BALANCED_BY_CATEGORY):
    ...
    def choose_index(label, forbidden=None):
        ...
        candidates = [int(idx) for idx in label_to_indices[int(label)]
                      if int(idx) not in used_indices and int(idx) not in forbidden]
        if not candidates:
...
def save_pair_metadata(pair_list, save_path):
    ...
    for pair_id, (src_idx, tgt_idx, src_lbl, tgt_lbl) in enumerate(pair_list):
        rows.append({
            ...
            "src_idx": int(src_idx),
            "tgt_idx": int(tgt_idx),
            "src_label": int(src_lbl),
            "tgt_label": int(tgt_lbl),
            "cross_category": bool(src_lbl != tgt_lbl),
...
def _run_shapenet_pair(test_idx, pair_tuple, all_pcs, gpu_id=None):
    """运行一个 ShapeNet pair；供单进程和多 GPU worker 复用。"""
    global CURRENT_SOURCE_PC, CURRENT_TARGET_PC
...
def _run_shapenet_pair_chunk(indexed_pair_chunk, gpu_id):
    """一个 GPU 子进程加载 ShapeNet 一次，然后顺序处理分配到的多个 pair。"""
    _configure_worker_gpu(gpu_id)
...
if __name__ == "__main__":
    ...
    pair_list = generate_shapenet_pairs(
        all_pcs_parent,
        all_labels_parent,
        num_pairs=NUM_PAIRS,
        seed=PAIR_SEED,
    ...
    if use_multi:
        ...
        # 释放父进程中的大数组引用；每个 worker 会独立加载一次 ShapeNet。
